...
class RoutingSlipProcessNextTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_process_next_on_empty_raises_error(self):
...
class RoutingSlipUndoLastTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_undo_last_success(self):
        ...
        result = await slip.undo_last()

        self.assertTrue(result)
        self.assertFalse(slip.is_in_progress)
        self.assertEqual(SuccessActivity.compensate_count, 1)

    async def test_undo_last_on_empty_raises_error(self):
    ...
    async def test_undo_multiple_items(self):
...
class RoutingSlipUriTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_compensation_uri_returns_last_activity_queue(self):
...
class RoutingSlipFullSagaTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_failed_saga_with_compensation(self):
        ...
        while not slip.is_completed:
            if not await slip.process_next():
                break
