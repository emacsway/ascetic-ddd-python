...
async def user_id_generator(session: ISession, query=None, position: int = -1) -> int:
...
async def email_generator(session: ISession, query=None, position: int = -1) -> str:
...
class UserProviderAutoIncrement(AggregateProvider[dict, User, int, UserId]):
    """
...
class UserProviderPresetPK(AggregateProvider[dict, User, int, UserId]):
    """
...
class AggregateProviderAutoIncrementTestCase(IsolatedAsyncioTestCase):
    ...
    async def test_create_inserts_new_aggregate(self):
        """output() should return new aggregate when ID is auto-incremented."""
    ...
    async def test_is_complete_true_after_create(self):
        """is_complete() should return True after output() and _output should be set."""
        repository = StubRepository(auto_increment_start=1)
        provider = UserProviderAutoIncrement(repository)
        provider.provider_name = 'user'
        session = MockSession()
    ...
    async def test_id_provider_updated_after_create(self):
        """id_provider should be updated with auto-incremented ID after populate()."""
...
class AggregateProviderPresetPKTestCase(IsolatedAsyncioTestCase):
    ...
    async def test_create_with_preset_id(self):
        """output() should use ID generated by ValueProvider."""
        repository = StubRepository()
    ...
    async def test_is_complete_true_after_create_preset_pk(self):
        """is_complete() should return True after output() with pre-set PK."""
        repository = StubRepository()
        provider = UserProviderPresetPK(repository)
        provider.provider_name = 'user'
        session = MockSession()
    ...
    async def test_id_provider_complete_before_create(self):
        """id_provider should be complete before output() with pre-set PK."""
        repository = StubRepository()
        provider = UserProviderPresetPK(repository)
        provider.provider_name = 'user'
        session = MockSession()
    ...
    async def test_reuse_existing_aggregate(self):
        """output() should reuse existing aggregate if found by ID."""
        repository = StubRepository()
        session = MockSession()
